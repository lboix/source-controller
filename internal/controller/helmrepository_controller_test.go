...
func TestHelmRepositoryReconciler_ociMigration(t *testing.T) {
	...
	t.Run("migrates newly created object with finalizer", func(t *testing.T) {
		g := NewWithT(t)
	...
	t.Run("migrates updated object with finalizer", func(t *testing.T) {
		g := NewWithT(t)

		hr := hr.DeepCopy()
		g.Expect(testEnv.Get(ctx, hrKey, hr)).ToNot(HaveOccurred())
	...
	t.Run("migrates deleted object with finalizer", func(t *testing.T) {
		g := NewWithT(t)
		...
		g.Eventually(func() bool {
			...
			return false
		}, timeout*2).Should(BeTrue()) // NOTE: Double the timeout to provide enough time for complete deletion.
	})
}
